...
public interface JpaFriendRepository extends JpaRepository<FriendEntity, FriendKey> {

    List<FriendEntity> findByFromUserId(String fromUserId);

    List<FriendEntity> findByToUserId(String toUserId);

    @Query("SELECT t1 FROM FriendEntity t1 JOIN FriendEntity t2 ON t1.fromUserId = t2.toUserId AND t1.toUserId = t2.fromUserId WHERE t1.fromUserId = :fromUserId")
    List<FriendEntity> findFriend(String fromUserId);

    @Query("SELECT t1 FROM FriendEntity t1 JOIN FriendEntity t2 ON t1.fromUserId = t2.toUserId AND t1.toUserId != t2.fromUserId WHERE t1.toUserId = :toUserId")
